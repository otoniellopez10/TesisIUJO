...
class persona {
    ...
    public function getOneByEmail($email) {
        ...
        $sql = "SELECT
                    ...
                    JOIN $this->tableUsuario AS u on u.id = p.usuario_id
                    JOIN $this->tableRol AS r on r.id = u.rol_id
                WHERE u.email = '$email' AND r.id IN (2,3)"; //que solo muestre colaboradors (2) y usuarios (3), mas no el admin (1)
        return $db->get_row($sql);
    }
    ...
    public function update($usuario_id, $nombre, $apellido, $cedula, $email, $telefono) {
        global $db;
    ...
    function getUsuarioVistasById($usuario_id){
        ...
        $sql = "SELECT
                    COUNT(DISTINCT v.libro_id) AS cantidad
                    FROM $this->tableVista v
                    JOIN $this->table AS p on p.id = v.usuario_id
